  ...
  <div id="myTabContent" class="tab-content">
    ...
    @if ($sentences->count()>0)
        @foreach ($sentences as $sentence)
            {{ '<li><span class="sentenceorg">' . $sentence->svnm . '</span><i class="j_listen icon-volume-up" data-w="' . urlencode(str_replace(array('.','"'),',',$sentence->seng)) . '"></i>' }}
            {{ '<ul><li><span class="sentencedes">' . $sentence->seng . '</span></li></ul></li>'  }}
        @endforeach
        @else
